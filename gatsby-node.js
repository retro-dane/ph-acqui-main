...
exports.onCreateNode = ({ node, actions, getNode }) => {
  ...
  if (node.internal.type === `MarkdownRemark`) {
    ...
    // Create a vehicle-specific ID from the file path
    // Convert /2022-honda-civic/ to just "2022-honda-civic"
    const vehicleId = value.replace(/^\//, '').replace(/\/$/, '')
    
    // Only create slugs for actual vehicle files, not schema.md
    if (vehicleId && vehicleId !== 'schema') {
      createNodeField({
        name: `slug`,
        node,
